...
export class MadiPipeline {
  ...
  async process(request: MadiInteractionRequest): Promise<MadiPipelineResult> {
    ...
    if (intent.requiresClarification) {
      return { requestId: request.requestId, timestamp: new Date().toISOString(), status: 'needs_input', intent, warnings: ['La intención no pudo determinarse con suficiente confianza.'] };
    }
    return { requestId: request.requestId, timestamp: new Date().toISOString(), status: 'completed', intent, data: [{ type: 'pipeline_ready', registeredCapabilities: this.capabilityRegistry.list().map((c) => c.id), executionBoundary: this.capabilityExecutor.constructor.name }] };
  }
}
